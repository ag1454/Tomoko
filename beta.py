...
# THIS IS USED FOR TESTING THIS IS NOT THE FINAL
# THE FINAL PROJECT IS tomoko.py
# THIS IS LIKE A BETA-TESTER OR SOMETHING
...
def timeUp(image):
    image.undraw()
    note = Image(Point(25, 30), "Gameover.gif")
    note.draw(win)
    return False
...
def actTomoko(win, status, standby_bars, tomoko_image1, tomoko_image2,
    ...
    timer = threading.Timer(2.0, timeUp)
    timer.start()

    if (choice == 1):
        timeUp(hungry_tomoko)
    elif (choice == 2):
        timeUp(sad_tomoko)
    else:
        timeUp(bored_tomoko)
